...
if (in_array($photo, $fav)) {
    ...
    echo "<div id='add_$sanitized_image'><i class='bi bi-heart-fill ms-3 fs-4 text-danger'></i>
                <button class='btn btn-outline-primary btn-sm ms-3 mb-2' hx-get='unselect.php?album=$album&photo=$safe_image' hx-trigger='click' hx-target='#add_$sanitized_image' hx-swap='outerHTML'>Unselect</button>
                </div>";
} else {
    $fav[] = $photo;
    $fav = json_encode($fav);
